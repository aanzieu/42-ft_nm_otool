...
/*   Updated: 2019/03/18 16:07:55 by aanzieu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		print_hexadump(char *start, uint16_t nb, t_bool space, t_obj *obj)
{
	uint16_t	i;
	...
	while (i <= nb)
	{
        ...
        ft_putnbr_ull_base((0x000000f0 & (*(char *)start)) >> 4, "0123456789abcdef", 1);
		ft_putnbr_ull_base((0x0000000f & (*(char *)start)), "0123456789abcdef", 1);
		if (!space || (i / 4 && !(i % 4)))
			ft_putchar(' ');
		start++;
		i++;
